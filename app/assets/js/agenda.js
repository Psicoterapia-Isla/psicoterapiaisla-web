...
async function searchPatients(term){
  if(!term||term.length<2){ suggestions.innerHTML=""; return; }
  ...
  snap.forEach(d=>{
    ...
    div.onclick=()=>{
      ...
      const h = Number(start.value.split(":")[0]);
      const m = Number(start.value.split(":")[1]);
...
async function getNextInvoiceNumber(therapistId){
  ...
  return await runTransaction(db,async tx=>{
    ...
    if(snap.exists()){
      ...
      tx.set(ref,{therapistId,year,lastNumber:1,createdAt:Timestamp.now()});
    }
...
async function maybeCreateInvoice(appointmentId,data){
  ...
  await updateDoc(doc(db,"appointments",appointmentId),{invoiceId:inv.id});
}
...
document.getElementById("save").onclick=async()=>{
  ...
  if(editingId){
    ...
    const ref=await addDoc(collection(db,"appointments"),{...data,createdAt:Timestamp.now()});
    id=ref.id;
  }
